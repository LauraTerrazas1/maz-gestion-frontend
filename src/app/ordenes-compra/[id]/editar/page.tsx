...
export default function EditarOrdenCompraPage() {
  ...
  const [itemsOriginalesIds, setItemsOriginalesIds] = useState<string[]>([]);
    ...
    async function cargarOrdenExistente() {
      try {
        ...
        if (data.estado !== "borrador") {
          setError(
            "Esta orden ya fue emitida y no puede editarse."
          );
          return;
        }
        ...
        setEventoProveedorId(
          data.evento_proveedor_id || ""
        );

        setFechaEmision(data.fecha_emision || "");
        setParticipacionEvento(
          data.participacion_evento || ""
        );
        setDescripcion(data.descripcion || "");
        setLugarEntrega(data.lugar_entrega || "");
        setFechaRequerida(data.fecha_requerida || "");
        ...
        const itemsCargados: ItemOC[] = Array.isArray(
          data.orden_compra_items
        )
          ? data.orden_compra_items.map(
            (item: {
              id: string;
              descripcion?: string;
  ...
  useEffect(() => {
    ...
    if (!fechaRequerida && eventoSeleccionado.fecha_inicio) {
      setFechaRequerida(eventoSeleccionado.fecha_inicio);
    }
  }, [eventoSeleccionado]);
  ...
  async function guardarOrden(
    ...
    try {
      ...
      // Actualizar la cabecera de la OC.
      await apiFetch(`/ordenes-compra/${ordenId}`, {
        method: "PUT",
        body: JSON.stringify({
      ...
      // Eliminar los ítems que el usuario quitó.
      const itemsEliminados =
        itemsOriginalesIds.filter(
          (id) => !idsActuales.includes(id)
        );

      for (const itemId of itemsEliminados) {
        await apiFetch(
          `/ordenes-compra/items/${itemId}`,
          {
            method: "DELETE",
          }
        );
      ...
      // Actualizar ítems existentes o crear nuevos.
      ...
      // Emitirla solo cuando el usuario presione "Emitir orden".
      if (estadoDestino !== estadoActual) {
        await apiFetch(
          `/ordenes-compra/${ordenId}/estado`,
          {
            method: "PUT",
            body: JSON.stringify({
      ...
      console.error(
        "Error actualizando la orden de compra:",
        error
      );

      setError(
        "No se pudo actualizar la orden de compra. Revisa los datos e inténtalo nuevamente."
      );
    } finally {
      setGuardando(false);
    }
      ...
      <main className="min-h-screen bg-[#F6F8FB] p-8">
        ...
        >
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Modifica los datos e ítems mientras la orden esté en borrador.
              </p>
            </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <Campo label="¿Se recibirá factura?">
              <div className="flex items-center gap-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                <button
                  ...
                  className={`relative h-6 w-11 rounded-full transition ${requiereFactura ? "bg-[#2F73D9]" : "bg-slate-300"
                    }`}
                >
                  <span
                    className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${requiereFactura ? "left-6" : "left-1"
                      }`}
                  />
                </button>
                ...
                <div>
                  <p className="text-sm font-semibold text-[#102033]">
                    {requiereFactura ? "Sí, se recibirá factura" : "No se recibirá factura"}
                  </p>
